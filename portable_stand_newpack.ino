...
int WrongCRC_Flag = 0; //флаг овечающий за реакцию на неверный crc
...
void Receive(){
  ...
  if(WrongCRC_Flag == 1){
    return;//несоответстие контрольной суммы
  }
  // Метод считывает пакеты в буфер
  if (buff[0] == 255 & buff[1] == 254){
    current.key = (int)buff[3];
    current.value1 = (buff[4]<<8)| buff[5];
    current.value2 = (buff[6]<<8)| buff[7];
    current.value3 = (buff[8]<<8)| buff[9];
    current.value4 = (buff[10]<<8)| buff[11];
    current.value5 = (buff[12]<<8)| buff[13];
    ptr = 1;
  }
  else{
    ptr = 0;   
...
int GetData(){
  ...
  for(int i = 2; i < 64; i++){
    ...
    if (buff[i] == 255 && CheckCRC(i-1) !=0){
      ...
      if (buff[i] == 255){
        res = i;
        if(CheckCRC(i-3) == 0){// i - 3 -> учет 2 байт начала, 2 конца, 1 контрольной суммы
          current.params = i - 6; // i + 1 - 3 (Начало + id) - 1(salt) - 1(crc) - 2(конец)
          break; 
        } 
        else{
...
void BuildMessage (byte cbyte, byte key, unsigned int value[]){
  ...
  printbuff[2] = (byte)key;
  printbuff[3] = (byte)cbyte;
  ...
  printbuff[pointer+1] = GetCRC((byte*)&data, sizeof(data));
...
void SendUART(){
    BuildMessage(current.condbyte, current.key, outbuff);
...
for (int i=0; i<current.params; i++){
  outbuff[i+1]=value[i];
}
SendUART();
}
